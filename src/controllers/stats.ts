...
export const getPieCharts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) charts = JSON.parse(myCache.get(key) as string);
    ...
    const productionCost = allOrders.reduce(
      ...
      0
    ); //In the case, where Company is paying shipping charges
    const burnt = allOrders.reduce((prev, order) => prev + (order.tax || 0), 0); //Assuming Tax is burnt
    const marketingCost = Math.round(grossIncome * (30 / 100)); //Assuming 30%
    const netMargin =
